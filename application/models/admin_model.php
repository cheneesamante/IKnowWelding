...
//            
    ...
    public function update_page_status($param_data = null) {
        try {
            ...
            if (null != $param_data or ! empty($param_data)) {
                ...
                $this->db->where('cms_id', $param_data['id']);
                $this->db->update('cms', $page_info);
                if ($this->db->affected_rows() > 0) {
                    $result_page_info = true;
                }
            }
        } catch (Exception $e) {
            $result_page_info = false;
        }
    ...
    public function update_page_info($param_data = null) {
        try {
            $result_page_info = null;
            if (null != $param_data or ! empty($param_data)) {
                $param_data['update_date'] = date('Y-m-d H:i:s');

                $this->db->where('cms_id', $param_data['cms_id']);
                $this->db->update('cms', $param_data);
                if ($this->db->affected_rows() > 0) {
                    $result_page_info = true;
                }
            }
