...
class Mahana_model extends CI_Model {
    ...
    public function get_recipient_id(){
         ...
         $sql = 'SELECT user_id, first_name, last_name FROM user_info WHERE active = 1 '
    ...
    public function get_recipient_find($param_recipient = '') {
        $user_id = $this->session->userdata('info')['user_id'];
        $sql = 'SELECT CONCAT(last_name, ", ", first_name, " ",middle_name, " - ", user_id) as name FROM user_info WHERE active = 1 '
                . 'and user_id NOT IN (' . $user_id . ') and CONCAT(last_name, ", ", first_name, " ",middle_name) LIKE "%'.$param_recipient.'%" ORDER BY last_name';

        $query = $this->db->query($sql);

        return $query->result_array();
    }
    public function get_recipient_findv2($param_recipient = '') {
        $result = false;
        $user_id = $this->session->userdata('info')['user_id'];
        $sql = 'SELECT user_id as id FROM user_info WHERE active = 1 '
                . 'and user_id NOT IN (' . $user_id . ') and CONCAT(last_name, ", ", first_name, " ",middle_name) LIKE "%'.$param_recipient.'%" or' 
                 . ' user_id = '.$param_recipient.' ORDER BY last_name LIMIT 1 ';
...
//user_info WHERE mp.user_id = user_id
//) AS to_name  FROM 
//msg_messages mm 
//LEFT JOIN msg_threads mt ON mm.thread_id = mt.id
//LEFT JOIN msg_participants mp
//ON mp.thread_id = mt.id WHERE mm.sender_id = ? ' ;
        $sql = 'SELECT *,
(SELECT CONCAT(last_name, ", ", first_name, " ",middle_name) FROM 
user_info WHERE mm.sender_id = user_id
) AS sender_name 
,
(SELECT CONCAT(last_name, ", ", first_name, " ",middle_name) FROM 
user_info WHERE mp.user_id = user_id
...
LEFT JOIN msg_threads mt ON mm.thread_id = mt.id
LEFT JOIN msg_participants mp
ON mp.thread_id = mt.id WHERE mp.user_id  = ?  AND  mm.sender_id <> ? ' ;
        ...
        $sql .= ' ORDER BY mt.id ' . $order_by. ', mm.cdate '. $order_by;
//        $sql .= ' AND mp.user_id <> '.$user_id.' ORDER BY mt.id ' . $order_by. ', mm.cdate '. $order_by;
//        $sql .= ' ORDER BY t.id ' . $order_by. ', m.cdate '. $order_by;
    ...
    function get_all_send($user_id, $full_thread = FALSE, $order_by = 'asc') {
//        $sql = 'SELECT m.*, s.status, t.subject, '.USER_TABLE_USERNAME .
//        ' FROM ' . $this->db->dbprefix . 'msg_participants p ' .
//        ' JOIN ' . $this->db->dbprefix . 'msg_threads t ON (t.id = p.thread_id) ' .
//        ' JOIN ' . $this->db->dbprefix . 'msg_messages m ON (m.thread_id = t.id) ' .
...
//LEFT JOIN msg_threads mt ON mm.thread_id = mt.id
//LEFT JOIN msg_participants mp
//ON mp.thread_id = mt.id WHERE mp.user_id = ? ' ;
//        
        $sql = 'SELECT *,
(SELECT CONCAT(last_name, ", ", first_name, " ",middle_name) FROM 
user_info WHERE mm.sender_id = user_id
...
LEFT JOIN msg_threads mt ON mm.thread_id = mt.id
LEFT JOIN msg_participants mp
ON mp.thread_id = mt.id WHERE mm.sender_id  = ?  AND  mp.user_id <> ? ' ;
...
//delete message start
    public function _delete_message($thread_id) {
        $this->db->delete('msg_messages', array('thread_id' => $thread_id));
...
//recent sent message start
    public function _recent_sent_message($user_id) {
        $data = array();
...
LEFT JOIN msg_threads mt ON mm.thread_id = mt.id
LEFT JOIN msg_participants mp
ON mp.thread_id = mt.id WHERE mp.user_id  = ?  AND  mm.sender_id <> ? ORDER BY mm.cdate DESC LIMIT 1';
...
//recent sent message start
    public function _recent_message($user_id) {
        $data = array();
...
LEFT JOIN msg_threads mt ON mm.thread_id = mt.id
LEFT JOIN msg_participants mp
ON mp.thread_id = mt.id WHERE mm.sender_id  = ?  AND  mp.user_id <> ? ORDER BY mm.cdate DESC LIMIT 1';
